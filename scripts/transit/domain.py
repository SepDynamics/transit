...
class TransitSnapshotService:
    ...
    def snapshot(self, *, scope: str = "all", trace_id: str | None = None, now_ms: int | None = None) -> Dict[str, Any]:
        return self._build_snapshot(scope=scope, trace_id=trace_id, now_ms=now_ms)

    def health(self, *, scope: str = "all", trace_id: str | None = None, now_ms: int | None = None) -> Dict[str, Any]:
        snapshot = self._build_snapshot(scope=scope, trace_id=trace_id, now_ms=now_ms)
        return snapshot["health"]

    def entities(self, *, scope: str = "all", trace_id: str | None = None, now_ms: int | None = None) -> Dict[str, Any]:
        snapshot = self._build_snapshot(scope=scope, trace_id=trace_id, now_ms=now_ms)
        return snapshot["entities"]

    def regimes(self, *, scope: str = "all", trace_id: str | None = None, now_ms: int | None = None) -> Dict[str, Any]:
        snapshot = self._build_snapshot(scope=scope, trace_id=trace_id, now_ms=now_ms)
        return snapshot["regimes"]

    def incidents(self, *, scope: str = "all", trace_id: str | None = None, now_ms: int | None = None) -> Dict[str, Any]:
        snapshot = self._build_snapshot(scope=scope, trace_id=trace_id, now_ms=now_ms)
        return snapshot["incidents"]

    def history(self, *, entity_id: str, scope: str = "all", trace_id: str | None = None, limit: int = 72, now_ms: int | None = None) -> Dict[str, Any]:
        snapshot = self._build_snapshot(scope=scope, trace_id=trace_id, now_ms=now_ms)
        vehicles = snapshot["entities"]["vehicles"]
        vehicle = next((row for row in vehicles if row["entity_id"] == entity_id), None)
        corridor_entity_id = (
            str(vehicle.get("corridor_entity_id") or "")
            if vehicle
            else ""
        )
        regime = next(
            (row for row in snapshot["regimes"]["regimes"] if str(row.get("entity_id") or "") == corridor_entity_id),
            None,
        )
        observations = [vehicle["observation"]] if vehicle and vehicle.get("observation") else []
    ...
    def _build_snapshot(self, *, scope: str = "all", trace_id: str | None = None, now_ms: int | None = None) -> Dict[str, Any]:
        del scope, trace_id
        source_stamp = self._source_stamp()
        built_at = float(self._snapshot_cache.get("built_at") or 0.0)
        if now_ms is None and self._snapshot_cache.get("stamp") == source_stamp and (time.monotonic() - built_at) <= 2.0:
        ...
        bundle = merge_realtime_bundles(self.cfg.system_name, *bundles) if bundles else TransitRealtimeBundle(
        ...
        augmented_vehicles = _attach_trip_update_delays(catalog, bundle.vehicles, augmented_trip_updates)
        ...
        if now_ms is None:
            self._snapshot_cache = {"stamp": source_stamp, "built_at": time.monotonic(), "payload": payload}
        return payload
    ...
    def _load_catalog(self, source: str) -> GTFSStaticCatalog:
        ...
        catalog = load_gtfs_catalog(source, feed_label=Path(source).stem or "transit-feed")
...
def _enrich_trip_updates(
    ...
    for update in trip_updates:
        ...
        for stop_update in update.stop_time_updates:
            ...
            if stop_payload["arrival_delay_seconds"] is None and stop_payload["arrival_time_unix"] is not None:
            ...
            if stop_payload["departure_delay_seconds"] is None and stop_payload["departure_time_unix"] is not None:
            ...
            for candidate in [stop_payload["arrival_delay_seconds"], stop_payload["departure_delay_seconds"]]:
                if candidate is None:
                    continue
                fallback_delays.append(int(candidate))
                if event_ms is not None and (now_ms - lookback_ms) <= event_ms <= (now_ms + lookahead_ms):
                    derived_delay_candidates.append((abs(event_ms - now_ms), int(candidate)))
        ...
        if payload.get("delay_seconds") is None:
            if derived_delay_candidates:
                ...
                payload["delay_seconds"] = max(fallback_delays, key=lambda value: abs(value))
...
def _trip_update_is_relevant(
    ...
    *,
    ...
    if schedule_window:
        ...
        if schedule_end_ms >= (now_ms - lookback_ms) and schedule_start_ms <= (now_ms + lookahead_ms):
            return True
        return False
    realtime_window = _trip_update_realtime_window_ms(update)
    if realtime_window:
        return realtime_window[1] >= (now_ms - lookback_ms) and realtime_window[0] <= (now_ms + lookahead_ms)
...
def _trip_update_chain_rank(
    ...
    *,
    ...
    is_live_vehicle = int(bool(update.vehicle_id and update.vehicle_id in live_vehicle_ids))
...
def _trip_update_realtime_window_ms(update: TransitTripUpdateObservation) -> Optional[Tuple[int, int]]:
    timestamps_ms: List[int] = []
    for stop_update in update.stop_time_updates:
        for candidate in [stop_update.arrival_time_unix, stop_update.departure_time_unix]:
...
def _trip_update_distance_from_now_ms(
    ...
    *,
    ...
    if schedule_window:
        midpoint_ms = int(((schedule_window[0] * 1000) + (schedule_window[1] * 1000)) / 2)
        return abs(midpoint_ms - now_ms)
...
def _stop_update_event_time_ms(
    ...
    *,
    ...
    timestamps_ms: List[int] = []
    for candidate in [stop_payload.get("arrival_time_unix"), stop_payload.get("departure_time_unix")]:
        if candidate is not None:
            timestamps_ms.append(int(candidate) * 1000)
    if timestamps_ms:
...
def _score_routes(
    *,
    ...
    feed_timezone: str,
) -> Tuple[List[Dict[str, Any]], List[TransitRegimeRecord], List[TransitIncidentRecord]]:
    ...
    for alert in alerts:
        if alert.route_ids:
            for route_id in alert.route_ids:
                matching_keys = [group_key for group_key in route_groups if group_key[0] == route_id]
...
def _partition_route_rows(
    ...
    active_lines = [row for row in route_rows if row.get("activity_status") == "active_now"]
    scheduled_later_lines = [row for row in route_rows if row.get("activity_status") == "scheduled_later"]
    inactive_lines = [row for row in route_rows if row.get("activity_status") == "inactive"]
...
def _compute_route_metrics(
    *,
    ...
    delays = [int(vehicle.delay_seconds or 0) for vehicle in vehicles if vehicle.delay_seconds is not None]
    if not delays:
        delays = [int(update.delay_seconds or 0) for update in trip_updates if update.delay_seconds is not None]
    ...
    delay_spread = (max(delays) - min(delays)) if len(delays) >= 2 else (max(delays) if delays else 0)
    route_vehicles = sorted(
        vehicles,
        key=lambda row: (
            row.current_stop_sequence if row.current_stop_sequence is not None else math.inf,
            row.timestamp_ms,
            row.vehicle_id,
        ),
    )
    ...
    for left, right in zip(route_vehicles, route_vehicles[1:]):
        ...
        if abs(right.current_stop_sequence - left.current_stop_sequence) <= 1:
            convoy_pairs += 1
    compressed_headway_share = convoy_pairs / max(1, len(route_vehicles) - 1) if len(route_vehicles) > 1 else 0.0
    terminal_backlog = sum(
        1
        for vehicle in route_vehicles
        if (
            (vehicle.current_stop_sequence is not None and vehicle.current_stop_sequence <= 2)
    ...
    latest_timestamp_ms = max(
        [row.timestamp_ms for row in route_vehicles] + [row.timestamp_ms for row in trip_updates] + [row.timestamp_ms for row in alerts] + [0]
    )
    feed_age_seconds = max(0.0, (now_ms - latest_timestamp_ms) / 1000.0) if latest_timestamp_ms else float(stale_after_seconds)
    position_coverage = sum(1 for row in route_vehicles if row.latitude is not None and row.longitude is not None) / max(1, len(route_vehicles))
    trip_update_coverage = min(1.0, len(trip_updates) / max(1, len(route_vehicles))) if route_vehicles else 0.0
    ...
    return {
        ...
        "alert_only": bool(active_alert_count and not route_vehicles and not trip_updates),
...
def _classify_route(metrics: Dict[str, Any]) -> Tuple[str, List[str]]:
    ...
    if metrics["feed_age_seconds"] >= metrics["stale_after_seconds"] and not metrics["observations_present"]:
    ...
    if metrics["alert_only"]:
        ...
        if metrics["active_alert_count"] >= 2:
            reasons.extend(["service_degraded", "service_alert_active", "stacked_alerts"])
            return "service_degraded", reasons
        return "healthy", ["healthy", "alert_only"]
    if metrics["trip_updates_only"] and metrics["feed_age_seconds"] < metrics["stale_after_seconds"]:
        if _has_strong_trip_update_delay_signal(metrics):
            reasons.extend(["service_degraded", "trip_updates_only", "persistent_delay"])
            return "service_degraded", reasons
    ...
    if (
        ...
        return "feed_incoherent", reasons
    if metrics["terminal_backlog_count"] >= 3 and metrics["median_delay_seconds"] >= 180:
        reasons.extend(["terminal_congestion", "terminal_backlog", "late_departures"])
        return "terminal_congestion", reasons
    if metrics["dwell_overrun_share"] >= 0.35 and metrics["median_delay_seconds"] >= 120:
        reasons.extend(["stop_dwell_instability", "dwell_overrun"])
        return "stop_dwell_instability", reasons
    if _supports_headway_regimes(metrics) and metrics["compressed_headway_share"] >= 0.75 and metrics["p90_delay_seconds"] >= 420:
        reasons.extend(["headway_collapse", "convoy_compression", "late_tail"])
        return "headway_collapse", reasons
    if _supports_headway_regimes(metrics) and metrics["compressed_headway_share"] >= 0.5 and metrics["median_delay_seconds"] >= 120:
    ...
    if (
        ...
        or (
            ...
            and metrics["median_delay_seconds"] >= max(180, int(service_delay_threshold * 0.75))
            and _has_alert_corroboration(metrics)
        )
        or (
...
def _service_delay_threshold(metrics: Dict[str, Any]) -> int:
    ...
    if route_mode in {"light_rail", "subway"}:
        return int(max(300.0, min(600.0, scheduled_headway * 0.5 if scheduled_headway else 300.0)))
    if route_mode == "bus":
        return int(max(420.0, min(900.0, scheduled_headway * 0.75 if scheduled_headway else 420.0)))
    if route_mode in {"commuter_rail", "ferry"}:
        return int(max(900.0, min(1800.0, scheduled_headway * 0.5 if scheduled_headway else 900.0)))
    return int(max(420.0, min(900.0, scheduled_headway * 0.6 if scheduled_headway else 420.0)))


def _warn_delay_threshold(metrics: Dict[str, Any]) -> int:
    route_mode = str(metrics.get("route_mode") or "other")
    multiplier = 1.3 if route_mode in {"light_rail", "subway"} else 1.45
    return int(max(_service_delay_threshold(metrics) * multiplier, 480 if route_mode in {"light_rail", "subway"} else 600))
...
def _has_strong_trip_update_delay_signal(metrics: Dict[str, Any]) -> bool:
    ...
    route_mode = str(metrics.get("route_mode") or "other")
    if route_mode == "bus":
        return metrics["trip_update_count"] >= 4 or metrics["median_delay_seconds"] >= max(720, int(threshold * 1.5))
    if route_mode in {"commuter_rail", "ferry"}:
        return metrics["trip_update_count"] >= 2 and metrics["median_delay_seconds"] >= max(1200, int(threshold * 1.2))
...
def _has_alert_corroboration(metrics: Dict[str, Any]) -> bool:
    ...
    if route_mode in {"commuter_rail", "ferry"}:
        return metrics["trip_update_count"] >= 2 or metrics["high_impact_alert_count"] >= 1
    return metrics["vehicle_count"] >= 1 or metrics["trip_update_count"] >= 2 or metrics["high_impact_alert_count"] >= 1
...
def _warn_riders_corroborated(metrics: Dict[str, Any]) -> bool:
    ...
    if route_mode in {"light_rail", "subway"}:
        if not metrics["scheduled_service_active"] and metrics["median_delay_seconds"] < _warn_delay_threshold(metrics):
            return False
        return (
...
def _recommended_action(regime: str, metrics: Dict[str, Any]) -> str:
    ...
    if regime == "corridor_unstable":
        if metrics["median_delay_seconds"] >= 240 or metrics["compressed_headway_share"] >= 0.65:
...
def _hazard_components(metrics: Dict[str, Any]) -> Dict[str, float]:
    ...
    delay_factor = clamp(float(metrics["median_delay_seconds"]) / max(1.0, float(_service_delay_threshold(metrics))))
    collapse_factor = clamp(float(metrics["compressed_headway_share"])) if _supports_headway_regimes(metrics) else 0.0
    ...
    feed_factor = clamp(float(metrics["feed_age_seconds"]) / max(1.0, float(metrics["stale_after_seconds"])))
    instability_factor = clamp(float(metrics["delay_spread_seconds"]) / max(360.0, scheduled_headway))
    return {
...
def _build_provenance(metrics: Dict[str, Any], components: Dict[str, float]) -> Tuple[float, Dict[str, Any]]:
    coverage = (float(metrics["position_coverage"]) * 0.55) + (float(metrics["trip_update_coverage"]) * 0.45)
    freshness = 1.0 - clamp(float(metrics["feed_age_seconds"]) / max(1.0, float(metrics["stale_after_seconds"])))
    signal_agreement = max(components.values(), default=0.0)
    confidence = clamp((0.45 * coverage) + (0.25 * freshness) + (0.30 * signal_agreement))
...
def _build_recurring_signatures(route_regimes: Sequence[TransitRegimeRecord]) -> List[Dict[str, Any]]:
...
def _route_upcoming_service_reason(
    ...
    *,
    ...
    current_window = catalog.route_service_window(route_id, direction_id=direction_id, service_date=current_date)
    if current_window:
    ...
    next_window = catalog.route_service_window(route_id, direction_id=direction_id, service_date=next_date)
    if next_window:
...
def _summarize_route_alerts(alerts: Sequence[TransitAlertObservation]) -> Dict[str, int]:
...
def _alert_is_high_impact(alert: TransitAlertObservation) -> bool:
    ...
    return any(
        keyword in text
        for keyword in ("delay", "behind schedule", "shuttle", "replace service", "detour", "no service", "terminate")
    )
