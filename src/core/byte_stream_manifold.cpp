 ...
 *   stability, entropy, rupture, a derived damping hazard, and a bucketed
 ...
 * - rupture comes from the structural engine's regime-shift ratio.
 * - hazard_lambda is a damping score derived from StructuralOptions rather than
 *   an operator-facing incident score.
...
double compute_entropy(const std::vector<uint8_t> &values) {
  if (values.empty()) {
    return 0.0;
  }
...
double compute_coherence(const std::vector<uint8_t> &values) {
  if (values.size() < 2) {
    return 1.0;
  }
  ...
  return clamp01(1.0 - (static_cast<double>(transitions) /
                        static_cast<double>(values.size() - 1)));
}

double compute_stability(const std::vector<uint8_t> &values) {
  if (values.size() < 2) {
    return 1.0;
  }
  double diff_sum = 0.0;
  for (size_t idx = 1; idx < values.size(); ++idx) {
    diff_sum +=
        std::abs(static_cast<double>(values[idx]) - static_cast<double>(values[idx - 1])) /
        255.0;
  }
  const double mean_diff = diff_sum / static_cast<double>(values.size() - 1);
  return clamp01(1.0 - std::min(1.0, mean_diff / 0.35));
}

double compute_rupture(const std::vector<uint8_t> &values) {
  if (values.size() < 2) {
    return 0.0;
  }
  const size_t split = std::max<size_t>(1, values.size() / 2);
  const auto mean_normalized = [](const auto begin, const auto end) {
    if (begin == end) {
      return 0.0;
    }
    double total = 0.0;
    ...
    for (auto it = begin; it != end; ++it) {
      total += static_cast<double>(*it) / 255.0;
      count += 1;
    }
    return count > 0 ? (total / static_cast<double>(count)) : 0.0;
  };

  const double first_mean = mean_normalized(values.begin(), values.begin() + static_cast<std::ptrdiff_t>(split));
  const double second_mean = mean_normalized(values.begin() + static_cast<std::ptrdiff_t>(split), values.end());
  return clamp01(std::abs(first_mean - second_mean) / 0.45);
}

double compute_hazard(double stability, double entropy, double rupture) {
  return clamp01((1.0 - stability) * 0.35 + clamp01(entropy / 2.4) * 0.35 +
                 rupture * 0.30);
}
...
ByteStreamManifold analyze_byte_stream(const std::vector<uint8_t> &bytes,
  ...
  if (bytes.size() * 8 < config.window_bits) {
    return manifold;
  }
  ...
  for (size_t start = 0; start + window_bits <= manifold.summary.analysed_bits;
    ...
    const size_t byte_start = start / 8;
    const size_t byte_count = std::max<size_t>(1, window_bits / 8);
    std::vector<uint8_t> window_bytes(
        bytes.begin() + static_cast<std::ptrdiff_t>(byte_start),
        bytes.begin() + static_cast<std::ptrdiff_t>(byte_start + byte_count));

    const double entropy = compute_entropy(window_bytes);
    const double coherence = compute_coherence(window_bytes);
    const double stability = compute_stability(window_bytes);
    const double rupture = compute_rupture(window_bytes);
    const double lambda_hazard = compute_hazard(stability, entropy, rupture);
    const bool collapsed = rupture >= config.structural_options.collapse_threshold;
