...
StructuralResult EntropyProcessor::analyze(const std::vector<uint8_t> &bits) {
  ...
  bitspace::DampedValue trajectory;
  ...
  if (!bits.empty() && bits.size() > 10) {
    trajectory = integrateFutureTrajectories(bits, 0);
    has_trajectory = true;
    double trajectory_confidence = matchKnownPaths(trajectory.path);
    ...
    if (std::abs(trajectory.final_value) < 2.0) {
      float stability_factor =
          1.0f / (1.0f + 0.1f * std::abs(static_cast<float>(trajectory.final_value)));
  ...
  const double event_stability =
      std::fmax(0.0, std::fmin(1.0, 1.0 - (0.7 * result.rupture_ratio) -
                                        (0.3 * result.oscillation_ratio)));
  const double entropy_stability = std::fmax(0.0, std::fmin(1.0, 1.0 - result.entropy));
  const double trajectory_stability =
      has_trajectory ? trajectory.confidence : event_stability;

  result.stability = std::fmax(
      0.0, std::fmin(1.0, 0.45 * event_stability + 0.35 * trajectory_stability +
                               0.20 * entropy_stability));
  result.confidence = std::fmax(
      0.0, std::fmin(1.0, 0.55 * result.coherence + 0.45 * result.stability));
